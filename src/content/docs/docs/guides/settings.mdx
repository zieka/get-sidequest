...
**CLI Path** -- Path to the Gemini CLI executable. Sidequest auto-detects the binary and shows its status (Detected / Not Found). Set an absolute path if auto-detection fails.

Gemini CLI is used by the [Council](/get-sidequest/docs/guides/council/) extension as an alternative agent for second-opinion feedback.

### Codex CLI

Path settings for Codex CLI are shown but currently marked as coming soon.
...
| **Council** | Get [second-opinion feedback](/get-sidequest/docs/guides/council/) on the current conversation from an alternative AI agent. |
