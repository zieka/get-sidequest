---
title: Quest Types
description: Understand the difference between Research, Changes, and Review quests.
---
...
**Active** -- The quest has a live Claude Code session. For Changes/Review quests, the worktree and branch are created when the quest first becomes active. You can rename an active quest's title by clicking it inline on the quest card.

**Recently Completed** -- When you complete a quest, it moves to the **Recently Completed** section at the bottom of the quest log. The session is killed and worktrees are cleaned up, but the quest remains visible for a retention period so you can review what was done. From here you can **restore** a quest back to active or **delete** it permanently. Completed quests are automatically pruned after the retention period expires.
