...
- **Link GitHub Issues** -- Select open issues to use as the quest prompt. Sidequest reads each issue's comments and linked issues to give Claude full context. When you select multiple issues, you can choose to create one quest per issue or a single quest addressing all of them.
...
Quests move through these states:

<Mermaid chart={`graph LR
  A[Backlog] -->|Start quest| B[Active]
  B -->|Pause| A
  B -->|Complete| C[Recently Completed]
  C -->|Restore| B`} />
...
- The quest moves to the **Recently Completed** section at the bottom of the quest log, where it remains visible for a retention period

From the Recently Completed section, you can **restore** a quest back to active or **delete** it permanently. Completed quests are automatically pruned after the retention period expires.
